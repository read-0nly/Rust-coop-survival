...
namespace Oxide.Plugins
{
	...
	public class DeployAnywhere : CovalencePlugin
	{
		...
		bool? OnGround(GroundWatch gw){
            BaseEntity component = gw.GetComponent<BaseEntity>();
			if (component)
			{
		...
		*/
		...
		BaseEntity OnPlayerInput(BasePlayer player, InputState input){
			...
			if(planner==null){
				if((player.GetHeldEntity().ToString().Contains("hammer.entity"))){
					...
					{
						be.Invoke(new Action(be.UpdateNetworkGroup), 5f);
				...
				float num = 1f;
                Item ownerItem = player.inventory.FindItemUID(planner.ownerItemUID);
				if (ownerItem != null)
				{
		...
		public void BreakConnection(BasePlayer player, IOEntity ioentity, int num, bool flag){
            ...
            IOEntity.IOSlot ioslot2 = flag ? ioentity2.outputs[ioslot.connectedToSlot] : ioentity2.inputs[ioslot.connectedToSlot];
			if (flag)
			{
