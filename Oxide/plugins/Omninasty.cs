...
namespace Oxide.Plugins{
	...
	public class Omninasty : RustPlugin{
		...
		object OnNpcTargetSense(BaseEntity owner, BaseEntity ent, AIBrainSenses brainSenses){
			...
			string otherFaction = (basePlayer!=null && basePlayer.HasPlayerFlag(BasePlayer.PlayerFlags.Connected) && config.Assignments.ContainsKey(basePlayer.userID.ToString())?config.Assignments[basePlayer.userID.ToString()]:(config.Assignments.ContainsKey(ent.GetType().ToString())?config.Assignments[ent.GetType().ToString()]:"Player"));
			
			if(selfFaction==otherFaction || (config.Alliances[selfFaction].Contains(otherFaction))){
