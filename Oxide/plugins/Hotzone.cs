	...
	public class Hotzone : CovalencePlugin{	
		...
		#region Faction logic		
			...
			[Command("hz_switch")] private void surv_aizend(IPlayer player, string command, string[] args){
				...
				SwitchFactionToAIZ(args[0],args[1]);
				SendChatMsg(bp, "<color=#FF0000>[AIZ Applied]</color>");
			}
				...
				*/
				...
				try{
					...
					if(Vector3.Distance(fc.gameObject.transform.position,bp.transform.position) < 10){
						...
						SendChatMsg(bp, "<color=#00FFFF>["+brain.CurrentState.ToString()+"] ");
						SendChatMsg(bp,brain.Navigator.StoppingDistance.ToString());
						if(brain.Navigator.Path==null){						
							AIInformationZone aiz = ActiveAIZ[fc.faction.ToString()];
							((HumanNPC)fc).VirtualInfoZone = aiz;
							brain.Navigator.Path = ((HumanNPC)fc).VirtualInfoZone.paths[0];
							if(brain.CurrentState is BaseAIBrain<HumanNPC>.BaseFollowPathState){
								//hn.Brain.CurrentState.StateEnter();
								((BaseAIBrain<HumanNPC>.BaseFollowPathState)brain.CurrentState).path = brain.Navigator.Path;
								((BaseAIBrain<HumanNPC>.BaseFollowPathState)brain.CurrentState).currentNodeIndex=UnityEngine.Random.Range(0,5);
							}							
						}
						SendChatMsg(bp,brain.Navigator.Path.transform.parent.name.ToString());
						SendChatMsg(bp,"CanUpdateMovement:" + (!(brain.Navigator.BaseEntity != null) || brain.Navigator.BaseEntity.IsAlive()).ToString()+"</color>");
						SendChatMsg(bp,"CanNavMes"+ brain.Navigator.CanUseNavMesh.ToString()+"</color>");
						brain.Navigator.Resume();
						SendChatMsg(bp,"CanNavMes"+ ("test").ToString()+"</color>");
						SendChatMsg(bp,brain.Navigator.BaseEntity.syncPosition.ToString()+"</color>");//
					}
				}catch(Exception e){
			...
			public void SwitchFactionToAIZ(string path, string faction){				
				AIInformationZone aiz = ActiveAIZ[path];		
				SwitchFactionToPath(aiz,faction);
			}
			public void SwitchFactionToPath(AIInformationZone aiz, string faction){			
			...
			*/
			...
			private bool swapSciRoamState(BasePlayer s){
				...
				try{
					...
					hn.Brain.Senses.maxRange=30f;
					hn.Brain.Navigator.CanUseNavMesh=true;
			...
			void initPlayer(BasePlayer player){
				...
				FactionController.changeScore(player, FactionController.FactionType.Scientist, 0.01f);
				FactionController.changeScore(player, FactionController.FactionType.Bandit, 0.01f);
				player.faction=FactionController.getNativeFaction(player);
				SaveConfig();
				}
			}
			...
			object OnFireworkDamage(BaseFirework bf, HitInfo info)
			{
				...
				{
					SwitchFactionToPath(AIInformationZone.GetForPoint(bf.transform.position, true),((BaseCombatEntity)info.Initiator).faction.ToString());
				}
				return null;
			...
			object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info){
				...
				try{
					if (info.damageTypes.Has(Rust.DamageType.Heat) && returnvar==true)
					{
						//Puts("Owning");
						...
						//Puts("Owned by "+((BasePlayer)info.Initiator).userID.ToString());
						return false;
					}					
				}catch(Exception e){
					...
					if(bp.IsConnected){		
							switch(((BaseCombatEntity)info.HitEntity).faction){
								case BaseCombatEntity.Faction.Bandit:
									FactionController.changeScore(bp, FactionController.FactionType.Scientist, 0.01f);
									FactionController.changeScore(bp, FactionController.FactionType.Bandit, -0.1f);
									FactionController.changeScore(bp, FactionController.FactionType.Both, -0.1f);
									bp.faction =FactionController.getNativeFaction(bp);				
									break;
									
								case BaseCombatEntity.Faction.Scientist:
									FactionController.changeScore(bp, FactionController.FactionType.Bandit, 0.01f);
									FactionController.changeScore(bp, FactionController.FactionType.Scientist, -0.1f);
									FactionController.changeScore(bp, FactionController.FactionType.Both, -0.1f);
									bp.faction =FactionController.getNativeFaction(bp);				
									break;
								case BaseCombatEntity.Faction.Player:
									...
									FactionController.changeScore(bp, FactionController.FactionType.Both, 0.005f);
									bp.faction =FactionController.getNativeFaction(bp);				
									break;
			...
			//*
			StateStatus? OnStateThink(BaseAIBrain<HumanNPC>.BaseFollowPathState state,float delta){
				//Puts(":");
				if(!(state.brain.OwningPlayer==null)){
					if(!(state.brain.OwningPlayer.transform==null)){
					
						if (Vector3.Distance(state.brain.OwningPlayer.transform.position,state.brain.transform.position) > 10f || state.currentWaitTime >= 120f)
						{//
							bool destRes = false;
							...
							Vector3 newPosition = state.brain.OwningPlayer.transform.position;
							newPosition.x += x;
							newPosition.z += y;
							float distance = Vector3.Distance(newPosition,state.brain.transform.position);
							if (distance<16f){
								 destRes=state.brain.Navigator.SetDestination(newPosition, global::BaseNavigator.NavigationSpeed.Slow, 0f, 0f);
				...
				if(state.brain.Navigator.Path==null){
						global::AIInformationZone forPoint = global::AIInformationZone.GetForPoint(state.brain.Navigator.BaseEntity.ServerPosition, true);
						if(forPoint.paths.Count()>00)
						state.brain.Navigator.Path = forPoint.paths[0];
				}
				
				if(state.brain.Navigator.Path==null && ActiveAIZ.ContainsKey(state.brain.Navigator.BaseEntity.faction.ToString())){
						global::AIInformationZone forPoint = ActiveAIZ[state.brain.Navigator.BaseEntity.faction.ToString()];
						state.brain.Navigator.Path = forPoint.paths[0];
				}
				...
				if(state.brain.Navigator.Path!=null && state.brain.Navigator.Path?.Points!=null){
					...
					float x = dist * Mathf.Cos(angle * Mathf.Deg2Rad);
					float y = dist * Mathf.Sin(angle * Mathf.Deg2Rad);
					Vector3 newPosition = state.brain.transform.position;
					newPosition.x += x;
					newPosition.z += y;
					float distance = Vector3.Distance(newPosition,state.brain.transform.position);
					if (distance<60 && distance> 5){
			...
			StateStatus? OnStateThink(BaseAIBrain<HumanNPC>.BasicAIState state,float delta){}
			object OnStateEnter(BaseAIBrain<HumanNPC>.BasicAIState state){}
			object OnStateLeave(BaseAIBrain<HumanNPC>.BasicAIState state){}
			
			
			StateStatus? OnStateThink(ScientistBrain.TakeCoverState state,float delta){}
		...
		private void OnServerInitialized()
        {
			...
			foreach(AIMovePointPath fc in GameObject.FindObjectsOfType<AIMovePointPath>()){
				GameObject.Destroy(fc.gameObject);
			}
			foreach(AIPoint fc in GameObject.FindObjectsOfType<AIPoint>()){
				GameObject.Destroy(fc.gameObject);
			}
			LoadConfig();
			initAIZ();
		...
		*/
		...
		public class FactionController : MonoBehaviour{
			...
			public static float getScore(BasePlayer bp, FactionType ft){
					if(bp==null)return 0.0f;
					if(ft==null)return 0.0f;
					if(bp.IsConnected != true) return 0.0f;
					FactionController fc = bp.GetComponent<FactionController>();
					if(fc==null)fc=bp.gameObject.AddComponent<FactionController>();
					ulong id= bp.userID;
					if(factionScores==null) return 0.0f;
					Dictionary<FactionType,float> selfFactions;
						factionScores.TryGetValue(id,out selfFactions);
					if(selfFactions==null) factionScores.Add(id, new Dictionary<FactionType,float>());
					float oldScore;//
			...
			public static BaseCombatEntity.Faction getNativeFaction(BasePlayer bp){
				if(getScore(bp,FactionType.Bandit)<0){
					if(getScore(bp,FactionType.Scientist)<0){
