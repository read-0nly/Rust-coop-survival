...
namespace Oxide.Plugins{
	...
	public class FactionHandler : RustPlugin{
		...
		public bool spawnForFaction(string faction){
			...
			if(config.FactionSpawnEntities[faction].Count>0 && spawnCounts[faction]>0)
            {
				int spawnAmount = (int)(config.FactionSpawnMinimums[faction]/(spawnCounts[faction]));
        ...
        static bool RadialPoint(BaseNavigator nav, Vector3 target, Vector3 self, float minDist = 5, float maxDist = 8)
        {
            ...
            newPosition.x += x;
            newPosition.z += y;
            newPosition.y = TerrainMeta.HeightMap.GetHeight(newPosition) + 5;
