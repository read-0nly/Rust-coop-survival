...
namespace Oxide.Plugins{
	...
	#region classes
		...
		public class OrderCell{
			...
			class FactionAnimalIdleState : BaseAIBrain.BaseIdleState{
				...
				public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity selfEntity)
				{
					if(!waitTimes.ContainsKey(this.brain)) waitTimes.Add(this.brain,0f);
					waitTimes[this.brain]+=delta;
					if(waitTimes[this.brain]<0.1f){return StateStatus.Running;}
			...
			class FactionAnimalRoamState : BaseAIBrain.BaseRoamState
        {
				private StateStatus status = StateStatus.Error;
			...
			class FactionAnimalChaseState : AnimalBrain.ChaseState{
				...
				public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity selfEntity)
				{
					...
					if(!waitTimes.ContainsKey(this.brain)) waitTimes.Add(this.brain,0f);
					waitTimes[this.brain]+=delta;
					if(waitTimes[this.brain]<0.5f){return StateStatus.Running;}
					waitTimes[this.brain]=0;
					if(this.brain.Navigator.BaseEntity.creatorEntity){
						if(this.brain.Navigator.BaseEntity.creatorEntity.transform){
							if(!(this.brain.Navigator.BaseEntity.creatorEntity as BasePlayer).mounted.IsSet()){
			...
			class FactionTakeCoverState : ScientistBrain.TakeCoverState{
                ...
                HumanNPC entity = brain.Navigator.BaseEntity as HumanNPC;
                ...
                AICoverPoint bestCoverPoint = informationZone.GetBestCoverPoint(entity.transform.position, hideFromPosition, minRange, bestCoverPointMaxDistance, entity, true);
					...
					if (!this.brain.Navigator.SetDestination(position, BaseNavigator.NavigationSpeed.Fast, 0f, 0f))
					{
						return false;
					}
			...
			class FactionChaseState : BaseAIBrain.BaseChaseState
			{
                ...
                BaseEntity baseEntity = this.brain.Events.Memory.Entity.Get(this.brain.Events.CurrentInputMemorySlot);
					if (baseEntity != null)
					{
						this.brain.Navigator.SetDestination(this.brain.PathFinder.GetRandomPositionAround(baseEntity.transform.position,5f, 10f), BaseNavigator.NavigationSpeed.Fast, 0.25f, 0f);
					}
				}
	...
	#region AI init
		...
		object OnDoRespawn(Ext.RustEdit.NPC.NPCSpawner npcSpawner){//*/){
	...
	#region Oxide Hooks
			...
			try{
				...
				if(bce.faction==bp.faction){
					...
					if((bce is HumanNPC)){
						...
						Item item2;
						item2 = ItemManager.CreateByItemID(((int)wi.item.info.itemid),1, 0UL);
						item2.OnVirginSpawn();
		...
		object OnEntityTakeDamage(BaseCombatEntity entity, HitInfo info){
				...
				try{
					...
					if(!info.damageTypes.Has(Rust.DamageType.Heat)){
						if(switcher < 0.4f){
							((HumanNPC)info.HitEntity).Brain.Navigator.SetDestination(info.HitEntity.transform.position+new Vector3(UnityEngine.Random.Range(-5.0f,5.0f),0,UnityEngine.Random.Range(-5.0f,5.0f)), BaseNavigator.NavigationSpeed.Fast, 0f, 0f);
							((HumanNPC)info.HitEntity).SetDucked(false);
							}
						else if(switcher < 0.7f){
							((HumanNPC)info.HitEntity).Brain.Navigator.SetDestination(info.HitEntity.transform.position, BaseNavigator.NavigationSpeed.Normal, 0f, 0f);
		...
		object OnGetBestTarget(HumanNPC hn){
			...
			float num = -1f;
			foreach (BaseEntity current in hn.Brain.Senses.Memory.Targets)
			{
		...
		object OnHelicopterUpdateTargets(PatrolHelicopterAI phai,Vector3 strafePos,bool flag,bool shouldUseNapalm){
			...
			{
				if (!basePlayer.HasPlayerFlag(BasePlayer.PlayerFlags.SafeZone) && Vector3Ex.Distance2D(phai.transform.position, basePlayer.transform.position) <= 150f &&basePlayer.faction!=BaseCombatEntity.Faction.Scientist)
				{
					bool flag4 = false;
					using (List<PatrolHelicopterAI.targetinfo>.Enumerator enumerator2 = phai._targetList.GetEnumerator())
					{
					...
					if (!flag4 && basePlayer.GetThreatLevel() > 0.5f && phai.PlayerVisible(basePlayer) && basePlayer.faction != BaseCombatEntity.Faction.Scientist)
					{
						if(basePlayer.faction != BaseCombatEntity.Faction.Scientist){
							phai._targetList.Add(new PatrolHelicopterAI.targetinfo(basePlayer, basePlayer));
						}
					}
				}
			}
			...
			for (int i = 0; i < brainsInSphere; i++)
			{
				...
				if(!(ent is ScientistNPC) && (ent is BasePlayer)){						
					...
					if(isVis){
						...
						if(!flag4)
							phai._targetList.Add(new PatrolHelicopterAI.targetinfo(ent, ply));
					}
